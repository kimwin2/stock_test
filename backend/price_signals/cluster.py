...
- generic한 이름(개별주, 특징주, 잡주, 기타, 기술주, 중소형주)은 금지합니다.
...
- **저가 상한가 종목 중시**: 주가 5,000원 이하이면서 상한가(upperLimit=true)를 기록한 종목은 테마 형성에 강한 시그널입니다. 단, 이런 종목들을 묶으려면 **실제로 같은 업종/재료**가 있어야 합니다. 업종이 완전히 다른 저가 상한가 종목들을 '저가주 급등' 같은 억지 테마로 묶지 마세요. 공통 재료가 없으면 각각 해당 업종 테마에 배정하거나 미분류로 남기세요.
...
JSON만 출력하세요."""
    ...
    user_prompt = (
        ...
        "이 종목들을 공통 테마별로 6~10개 씨앗 후보로 묶어주세요.\n\n"
        "출력 형식:\n"
        "{\n"
        '  "themes": [\n'
        "    {\n"
...
- 5~10개 후보만 남기고, 각 테마는 2~8종목까지만 포함하세요.
