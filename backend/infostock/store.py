...
LOCAL_SIGNAL_PATH = os.path.join(DEV_DIR, "infostock_themes.json")
DEFAULT_SIGNAL_KEY = "signals/infostock_themes_latest.json"
...
def _resolve_signal_key(signal_key: str | None = None) -> str:
    return (signal_key or os.getenv("INFOSTOCK_SIGNAL_S3_KEY", DEFAULT_SIGNAL_KEY)).strip()
...
def save_infostock_payload(
    ...
    print(f"[INFO] 인포스탁 테마 데이터를 {target}에 저장했습니다.")
    return target
