...
- 크롤링된 기사들을 분석하여 테마를 최대 7개까지 추출합니다 (정원 아님 — 근거 있는 만큼만).
...
1. **테마 추출 (최대 9개, 정원 아님)**: 오늘 실제로 주목받는 투자 테마를 최대 9개까지 선정합니다.
   - **개수를 채우지 마세요.** 근거가 확실한 테마가 4개뿐이면 4개만 내세요. 빈 자리를 메우려고
     오늘 시세가 확인되지 않은 테마를 지어내면, 그 테마는 뒤 단계에서 실제 등락률로 걸러져
     버려집니다. 억지로 채운 테마는 결과에 남지 않고 분석만 흐립니다
   ...
   - `와우넷 오늘의 특징주`에서 제목 날짜가 당일 또는 직전 거래일(주말·월요일은 금요일 포함)인 게시글의 `4. 강세업종`과 `5. 특징주`는 고신뢰 장중 시그널입니다. 뉴스와 겹치면 최우선 반영하고, 일부만 겹쳐도 강하게 반영하세요
   ...
   - **종합 판단**: 개미승리·인포스탁·가격 기반 후보 등 실측 시그널이 뒷받침하는 테마를 먼저 채우고, 뉴스 기사 분석만으로 도출한 테마는 그 다음입니다. 관련종목은 여러 소스를 종합하여 다양하게 구성하세요
   - **뉴스 단독 테마의 조건**: 외부 시그널 없이 기사만 보고 만든 테마는, 그 테마의 관련종목이 오늘 실제로 크게 오르고 있다는 근거(★ 표시 기사, 상한가·급등 언급 등)가 있을 때만 선정하세요. 근거가 "업황이 좋다" 수준이면 제외하세요
   - **테마명과 종목이 일치해야 합니다**: relatedStocks 는 그 테마명의 사업을 실제로 하는 기업이어야 합니다. 조선기자재 테마에 전력기기 회사를, 게임 테마에 게임을 만들지 않는 회사를 넣지 마세요. 넣을 종목이 없으면 그 테마를 빼세요
...
2. **유사 테마 병합 (반드시 준수)**:
   ...
   - 병합 후 최종 테마 수는 **최대 7개**입니다. 7개를 채울 의무는 없습니다 — 병합하고 근거 약한 것을 버린 결과가 4개면 4개로 끝내세요
   - **핵심 목표: 남은 테마가 서로 최대한 다양하고 겹치지 않아야 합니다**

3. **종목 중복 금지 (반드시 준수)**:
   - 한 종목은 전체 테마 통틀어 딱 1번만 등장해야 합니다
...
분석하여 오늘의 주도 테마를 최대 9개까지 추출한 뒤, 유사 테마를 병합하여 최대 7개로 정리해주세요.
개수는 정원이 아닙니다. 근거가 확실한 테마만 남기고, 모자라면 모자란 대로 두세요.
...
def analyze_themes(articles: list[dict], date_str: str = None) -> dict:
    ...
    try:
        ...
        # 개수는 정원이 아니다. 최종 노출 개수는 stock_data 의 종목 선정 게이트가 정한다
